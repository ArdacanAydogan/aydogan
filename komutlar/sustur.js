...
exports.run = (client, message, args) => {
  ...
  let modlog = guild.channels.find('name', 'aydogan-log');
  let muteRole = client.guilds.get(message.guild.id).roles.find('name', 'Cezalı');
  if (!modlog) return message.reply('`aydogan-log` kanalını bulamıyorum.').catch(console.error);
  if (!muteRole) return message.reply('`Cezalı` adlı bir rol bulamıyorum.').catch(console.error);
